...
import gc # 需要引入 gc 模块来释放每个 epoch 的内存

def get_epoch_num(filename):
    """从文件名中提取 epoch 编号，例如 epoch0_... -> 0"""
    match = re.search(r'epoch(\d+)_', os.path.basename(filename))
    if match:
        return int(match.group(1))
    return -1

def get_total_rows_from_files(files):
    """
    遍历指定的文件列表，提取文件名末尾数字（如 d34 中的 34）并累加。
    """
    total_rows = 0
    for filename in files:
        if filename.endswith(".npz"):
...
def train(Game, GameNet):
    ...
    if not npz_files:
        print("没有找到任何 .npz 数据文件，无法进行训练。")
        return

    # 解析所有存在数据的 epoch 范围
    epochs = [get_epoch_num(f) for f in npz_files]
    valid_epochs = [e for e in epochs if e != -1]
    if not valid_epochs:
        print("未能在文件名中找到标准的 epoch 编号格式(如 epoch0_xxx)。")
        return
    
    max_epoch = max(valid_epochs)
    print(f"检测到历史数据涵盖 epoch 0 到 {max_epoch}")

    # 初始化网络和优化器
    real_net = GameNet().to(device)
    print("初始化网络完毕。")

    # 检查并加载已有模型 (这里你可以选择是否要在模拟前清空旧模型)
    ...
    # ================= 核心修改：外层大循环 =================
    for sim_epoch in range(max_epoch + 1):
        print(f"\n==================================================")
        print(f"开始模拟训练阶段: Epoch {sim_epoch} / {max_epoch}")
        print(f"==================================================")
        
        # 1. 过滤出在当前 sim_epoch 及之前产生的所有文件
        available_files = [f for f in npz_files if 0 <= get_epoch_num(f) <= sim_epoch]
        if not available_files:
            print(f"阶段 Epoch {sim_epoch} 没有找到对应数据，跳过。")
            continue

        # 2. 统计当前阶段的累积总数据量，并计算 KataGo 窗口
        total_rows = get_total_rows_from_files(available_files)
        desired_rows = win_calc.calculate(total_rows)
        print(f"[数据窗口] 截至 Epoch {sim_epoch}，共产生总数据数: {total_rows}")
        print(f"[数据窗口] 根据公式，本次实际将抽取最新的数据数: {desired_rows}")

        # 3. 按时间戳倒序（新 -> 旧）排序
        sorted_files = sorted(available_files, key=get_timestamp, reverse=True)

        # 4. 加载数据（限定在 desired_rows 以内）
        all_data = []
        for file in tqdm(sorted_files, desc=f"加载 Epoch {sim_epoch} 数据", unit="file"):
            try:
        ...
        print(f"[数据加载] 实际加载入内存的数据条数: {len(all_data)}")
        if not all_data:
            print("加载的数据为空，跳过本 epoch。")
            continue
        ...
        print(f"[训练前] 验证集 Loss: {val_loss:.4f}")

        # 5. 执行当前 Epoch 的训练
        real_net.train()
        random.shuffle(train_data)
        
        pbar = tqdm(range(0, len(train_data), BATCH_SIZE), desc=f"训练 Epoch {sim_epoch}")
        for i, start in enumerate(pbar):
            end = min(start + BATCH_SIZE, len(train_data))
            batch_data = train_data[start:end]

            sizes, states, policy_targets, opp_policy_targets, value_targets = [], [], [], [], []
        ...
        # 当前阶段训练结束，验证并保存
        val_loss = validate(real_net, val_data, device)
        print(f"[训练后] 验证集 Loss: {val_loss:.4f}")
        
        # 我们用 sim_epoch 标识模型文件，方便你追踪
        model_pth_path = f'model/candidate-epoch{sim_epoch}-s{total_samples}-d{total_rows}.pth'
        model_onnx_path = f'model/candidate-epoch{sim_epoch}-s{total_samples}-d{total_rows}.onnx'
        model_pt_path = f'model/candidate-epoch{sim_epoch}-s{total_samples}-d{total_rows}.pt'

        save_3_nets_and_checkpoint(real_net, model_pth_path, model_onnx_path, model_pt_path, device, optimizer, total_samples)
        
        # 6. 显式释放内存，防止 OOM (Out Of Memory)
        del all_data
        del train_data
        del val_data
        gc.collect()
